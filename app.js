...
window.Shared = (function () {
  ...
  const HEADER_ALIASES = {
    ...
    stockStatus: ["location", "stockstatus", "status"],
  ...
  const TABLE_COLUMNS = [
    { key: "partNo", label: "Part Code", width: 130 },
    ...
    { key: "commodity", label: "Commodity", width: 150 },
    ...
    { key: "agingStatus", label: "Age", width: 110 },
  ...
  const state = {
    rawData: [],
    filters: { area: "", district: "", store: "", brand: "", productFamily: "", commodity: "", stockStatus: "", partNo: "", description: "" },
    tableSearch: "",
  ...
  const overlay = { remarks: {}, showroomVisits: {}, activity: [] };
  const overlayMeta = { sha: null, dirtyRemarks: new Set(), dirtyShowroom: new Set(), pendingActivity: [], saveTimer: null, saving: false };
  ...
  async function performOverlaySave(isRetry) {
    ...
    try {
      ...
      const msg = `Update remarks/showroom via dashboard \u2014 ${who} \u2014 ${new Date().toISOString()}`;
      ...
      if (e.status === 409 || e.status === 422) {
        // Conflict: someone else saved first. Pull latest, re-apply our dirty keys on top, retry once.
        try {
          setSaveStatus("saving", "Retrying\u2026");
          const remote = await GitHubService.getJson(OVERLAY_PATH);
          const remoteData = remote.data || { remarks: {}, showroomVisits: {}, activity: [] };
          ...
          const merged = [...overlayMeta.pendingActivity.filter((a) => !existingIso.has(a.iso)), ...(remoteData.activity || [])].slice(0, 50);
          remoteData.activity = merged;
          ...
          const session = getSession();
          const who = session ? session.name : "Someone";
          const newSha = await GitHubService.putJson(OVERLAY_PATH, overlay, `Merge update \u2014 ${who} \u2014 ${new Date().toISOString()}`, overlayMeta.sha);
          overlayMeta.sha = newSha;
          overlayMeta.dirtyRemarks.clear();
          overlayMeta.dirtyShowroom.clear();
          overlayMeta.pendingActivity = [];
  ...
  function hasUnsavedWork() {
    return overlayMeta.dirtyRemarks.size > 0 || overlayMeta.dirtyShowroom.size > 0 || overlayMeta.saving;
  }
  ...
  function wireAuthScreens() {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", async () => {
        ...
        setSession({ name, dept, loggedInAt: new Date().toISOString() });
        showScreen("appShell");
        await bootApp();
      });
    }
    ...
    if (adminBtn) {
      adminBtn.addEventListener("click", async () => {
        const key = prompt("Administrator key:");
        if (!key) return;
        try {
          await GitHubService.verifyAdmin(key);
          sessionStorage.setItem("over6_admin_key", key);
          document.getElementById("uploadLabel").style.display = "";
          adminBtn.textContent = "Admin Unlocked";
          adminBtn.classList.add("admin-unlocked");
          alert("Administrator access enabled for this browser session.");
        } catch (e) {
          alert("Could not verify the dashboard service.\n\n" + e.message);
        }
      });
    }
  }
  ...
  function initAuthGate() {
    wireAuthScreens();
    const session = getSession();
    if (!session || !session.name) { showScreen("loginScreen"); return; }
    showScreen("appShell");
    bootApp();
  }
  ...
  function resetFiltersAndView() {
    state.filters = { area: "", district: "", store: "", brand: "", productFamily: "", commodity: "", stockStatus: "", partNo: "", description: "" };
    state.tableSearch = "";
  ...
  function applyFilters(rows, excludeField) {
    ...
    return rows.filter((r) => {
      ...
      if (excludeField !== "stockStatus" && f.stockStatus && r.stockStatus !== f.stockStatus) return false;
  ...
  function refreshFilterOptions() {
    ...
    const statusScoped = applyFilters(state.rawData, "stockStatus");
    const statuses = uniqueSorted(statusScoped, "stockStatus");
    const sel = document.getElementById("f-status");
    const cur = state.filters.stockStatus;
    sel.innerHTML = `<option value="">All Statuses</option>` + statuses.map((s) => `<option value="${escapeHtml(s)}" ${s === cur ? "selected" : ""}>${escapeHtml(s)}</option>`).join("");
  }
  ...
  function renderScopeStrip(rows) {
    ...
    if (f.stockStatus) chips.push(`<span class="scope-chip">Status: <b>${escapeHtml(f.stockStatus)}</b></span>`);
  ...
  function renderTableHead() {
    ...
    row.innerHTML = TABLE_COLUMNS.filter((c) => !state.hiddenCols[c.key]).map((col) => {
      const isSorted = state.sort.field === col.key;
      const arrow = isSorted ? (state.sort.dir === "asc" ? "\u25B2" : "\u25BC") : "";
      return `<th data-key="${col.key}" style="width:${col.width}px" class="${col.numeric ? "num" : ""}">
  ...
  function renderTableBody(pageRows) {
    ...
    const cols = TABLE_COLUMNS.filter((c) => !state.hiddenCols[c.key]);
    if (!pageRows.length) {
    ...
    tbody.innerHTML = pageRows.map((r) => {
      return "<tr>" + cols.map((col) => {
        if (col.isRemark) {
          return `<td class="remark-cell"><input type="text" value="${escapeHtml(r.remark)}" data-partno="${escapeHtml(r.partNo)}" data-store="${escapeHtml(r.store)}" placeholder="Add remark\u2026"></td>`;
        }
        if (col.isMeta) return `<td class="meta-cell">${escapeHtml(r[col.key])}</td>`;
        if (col.key === "value") return `<td class="num">${fmtMoney(r.value)}</td>`;
        if (col.key === "closingBalance") return `<td class="num">${fmtInt(r.closingBalance)}</td>`;
        if (col.key === "agingStatus") return `<td><span class="status-pill ${statusPillClass(r.stockStatus)}">${escapeHtml(r.agingStatus)}</span></td>`;
        return `<td title="${escapeHtml(r[col.key])}">${escapeHtml(r[col.key])}</td>`;
      }).join("") + "</tr>";
    }).join("");

    tbody.querySelectorAll('input[data-partno]').forEach((input) => {
      // Debounced 'input' saves the data (so nothing is lost even without a
      // blur), but doesn't spam the activity log with every partial keystroke
      // pause. 'change' (fires on blur/Enter) does the same save AND logs the
      // action, once, with the final text.
      const debouncedSilentSave = debounce(() => handleRemarkEdit(input, { log: false }), 700);
      input.addEventListener("input", debouncedSilentSave);
      input.addEventListener("change", () => handleRemarkEdit(input, { log: true }));
      input.addEventListener("keydown", (e) => { if (e.key === "Enter") e.target.blur(); });
    });
  }

  function handleRemarkEdit(input, opts) {
    ...
    markRemarkDirty(key);
    if (shouldLog) logActivity(who, `updated remark for ${partNo} at ${store}${text ? ": \u201C" + text + "\u201D" : " (cleared)"}`);
  ...
  function renderColsMenu() {
    ...
    menu.innerHTML = TABLE_COLUMNS.map((c) => `<label><input type="checkbox" data-key="${c.key}" ${state.hiddenCols[c.key] ? "" : "checked"}> ${c.label}</label>`).join("");
  ...
  function rowsToAOA(rows) {
    const header = ["Area", "District", "Shop", "Part Code", "Description", "Brand", "Product Family", "Commodity",
      "Quantity", "Selling Price", "Inventory Value", "Stock Status", "Aging Status", "Remark", "Commented By", "Comment Date", "Comment Time", "Last Updated"];
    const body = rows.map((r) => [r.area, r.district, r.store, r.partNo, r.description, r.brand, r.productFamily, r.commodity,
      r.closingBalance, r.sellingPrice, r.value, r.stockStatus, r.agingStatus, r.remark, r.commentedBy, r.commentDate, r.commentTime, r.lastUpdated]);
    return [header, ...body];
  }
  ...
  async function handleUpload(file) {
    const adminKey = sessionStorage.getItem("over6_admin_key");
    if (!adminKey) {
      alert("Administrator access is required to upload a new inventory workbook. Use the Admin button first.");
      return;
    }
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        setLoading(true, "Backing up and committing Inventory.xlsx…");
        const invPath = "data/Inventory.xlsx";
        const result = await GitHubService.adminUploadBinary(
          invPath,
          e.target.result,
          `Upload new inventory export — ${new Date().toISOString()}`,
          adminKey
        );
        state.inventorySha = result.sha || null;
        ...
        const session = getSession();
        logActivity(session ? session.name : "Administrator", `uploaded a new inventory workbook (${newRows.length.toLocaleString()} rows)`);
        scheduleOverlaySave();
      } catch (err) {
        console.error(err);
        if (err.status === 401 || err.status === 403) sessionStorage.removeItem("over6_admin_key");
  ...
  function wireNav() {
    document.querySelectorAll(".nav-item[data-page]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        document.getElementById("uploadLabel").style.display = (btn.dataset.page === "dashboard" && sessionStorage.getItem("over6_admin_key")) ? "" : "none";
    ...
    document.getElementById("refreshBtn").addEventListener("click", async () => {
      ...
      try {
        ...
        if (window.Showroom) await window.Showroom.loadAndRender();
  ...
  function wireStaticControls() {
    ...
    document.getElementById("f-status").addEventListener("change", (e) => { state.filters.stockStatus = e.target.value; state.page = 1; renderAll(); });
  ...
  async function bootApp() {
    ...
    try {
      ...
      updateDataMeta();
      renderActivityLog();
      if (window.Showroom) await window.Showroom.init();
    } catch (e) {
      console.error(e);
