...
const GitHubService = (function () {
  ...
  function getConfig() {
    // Shared config.js is the primary configuration.
    // This keeps the existing token-based GitHub method while preventing
    // an old localStorage credential from overriding the current token.
    try {
    ...
    // Backward-compatible fallback for installations that do not use config.js.
    try {
