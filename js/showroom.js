...
window.Showroom = (function () {
  ...
  function renderTable() {
    ...
    tbody.querySelectorAll("select[data-field]").forEach((el) => {
      el.addEventListener("change", () => handleEdit(el, { log: true }));
    });
    tbody.querySelectorAll("input[data-field]").forEach((el) => {
      // Debounced silent save while typing, so a remark/team/date is never lost
      // if the user navigates away without blurring the field.
      const debouncedSilentSave = S.debounce(() => handleEdit(el, { log: false }), 700);
      el.addEventListener("input", debouncedSilentSave);
      // Real save + log on blur/Enter, with the final value.
      el.addEventListener("change", () => handleEdit(el, { log: true }));
      if (el.type === "text") el.addEventListener("keydown", (e) => { if (e.key === "Enter") e.target.blur(); });
    });
  }

  function handleEdit(el, opts) {
    ...
    if (shouldLog) S.logActivity(who, `updated ${showroom} showroom (${field}: ${el.value || "cleared"})`);
    renderKpis();
    ...
    // Patch just this row's "Updated By" / "Updated Date" cells in place
    // instead of rebuilding the whole table. A full rebuild here would
    // destroy and recreate every <input> in the table, including ones a
    // user might be mid-click/mid-type into on a sibling field in the same
    // row — a real focus-loss / lost-edit bug, not just a cosmetic issue.
    const tr = el.closest("tr");
    if (tr) {
      const metaCells = tr.querySelectorAll(".meta-cell");
      if (metaCells[0]) metaCells[0].textContent = row.updatedBy;
      if (metaCells[1]) metaCells[1].textContent = row.updatedDate;
    }
  }
