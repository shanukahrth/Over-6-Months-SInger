...
window.Shared = (function () {
  ...
  function initAuthGate() {
    ...
    if (!GitHubService.isConfigured()) {
      // If this deployment ships a filled-in js/config.js, use that shared
      // connection automatically so regular users never see the "Connect to
      // GitHub" screen — only whoever maintains config.js has to set it up,
  ...
  function renderTableBody(pageRows) {
    ...
    tbody.innerHTML = pageRows.map((r) => {
      return "<tr>" + cols.map((col) => {
        ...
        if (col.key === "agingStatus") return `<td class="${stickyClass}"><span class="status-pill ${statusPillClass(r.stockStatus)}">${escapeHtml(r.agingStatus)}</span></td>`;
