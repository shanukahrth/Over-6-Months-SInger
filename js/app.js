...
window.Shared = (function () {
  ...
  function wireAuthScreens() {
    document.getElementById("ghConnectBtn").addEventListener("click", async () => {
      ...
      const branchInput = document.getElementById("gh-branch").value.trim();
      const token = document.getElementById("gh-token").value.trim();
      const errEl = document.getElementById("ghSetupError");
      errEl.classList.add("hidden");
      if (!owner || !repo || !token) { errEl.textContent = "Please fill in owner, repo and token."; errEl.classList.remove("hidden"); return; }
      const btn = document.getElementById("ghConnectBtn");
      btn.disabled = true; btn.textContent = "Connecting\u2026";
      try {
        // Probe with a placeholder branch first just to validate owner/repo/token;
        // the repo metadata response tells us the *real* default branch, which we
        // use unless the user explicitly typed a different one. This avoids the
        // extremely common "typed main but the repo actually uses master" failure.
        const repoInfo = await GitHubService.testConnection({ owner, repo, branch: branchInput || "main", token });
        const branch = branchInput || repoInfo.default_branch || "main";
        GitHubService.setConfig({ owner, repo, branch, token });
        const existing = getSession();
        if (existing && existing.name) { showScreen("appShell"); await bootApp(); }
        else showScreen("loginScreen");
      } catch (e) {
        errEl.textContent = e.message || "Could not connect. Check the details and token permissions.";
        errEl.classList.remove("hidden");
      } finally {
        btn.disabled = false; btn.textContent = "Connect & Continue";
      }
    });

    document.getElementById("changeGhLink").addEventListener("click", () => {
      const cfg = GitHubService.getConfig();
      if (cfg) {
  ...
  function initAuthGate() {
    wireAuthScreens();
    if (!GitHubService.isConfigured()) { showScreen("githubSetupScreen"); return; }
    const session = getSession();
